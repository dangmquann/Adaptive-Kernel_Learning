...
class EasyMKL():
    ...
    def traceN(self, k):
        if isinstance(k.size, int):
            m = int(np.sqrt(k.size))  # Số dòng và số cột là căn bậc hai của tổng số phần tử
        else:
            m = k.size[0]  # Nếu k.size là tuple, lấy số dòng từ k.size[0]
        return sum([k[i,i] for i in range(m)]) / m
