...
class MultiHeadAttn(nn.Module):
    ...
    def forward(self, x, prior_guide=None):
        ...
        if prior_guide is not None:
            # logging.info(f"Using prior_guide for attention initialization: {prior_guide.shape}")
            attn_weights = torch.zeros(self.num_heads, N, N, device=x.device)
            with torch.no_grad():
                for h in range(self.num_heads):
                    attn_weights[h] = prior_guide
        else:
            # logging.info("No prior_guide provided, using learnable attention parameters")
            # Normalize attention scores
            if self.attention_normalizer == 'softmax':
                attn_weights = F.softmax(self.attn_params, dim=-1)
            else:
                attn_weights = torch.sigmoid(self.attn_params)
...
class ATK(nn.Module):
    ...
    def forward(self, data):
        ...
        prior_guide = data.prior_guide if hasattr(data, 'prior_guide') else None
        x, attentions = self.atk(x, prior_guide)
        logits = self.classifier(x).squeeze(-1)

        return logits, attentions

    def get_loss(self, logits, labels, attentions, train_mask=None):
        """
        ...
        if  len(attentions) > 1: 
            ...
            # logging.info(f"BCE loss: {loss}, KL regularization term: {reg_term.item()}")
            # loss = (1 - self.atk.reg_coef) * loss + self.atk.reg_coef * reg_term
            ...
            kernel_loss = 0
            loss = (1 - self.atk.reg_coef) * loss + self.atk.reg_coef * (kernel_loss + reg_term)
            print(
                f"Loss: {loss.item():.4f}, BCE: {bce.item():.4f}, Reg: {reg_term.item():.6f}") #, Kernel Loss: {kernel_loss:.4f}")
