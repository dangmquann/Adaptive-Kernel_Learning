...
def gen_test_adj_mat_tensor(data, trte_idx, parameter, metric="cosine"):
    ...
    adj = F.normalize(adj + I, p=1)
    adj = to_sparse(adj)
    # print("Adjacency matrix generated!", adj)
...
def adj_from_kernel_matrix(kernel_matrix, top_k=5, normalize=True):
    """
    Tạo adjacency matrix sparse tensor từ kernel matrix.
    Args:
        kernel_matrix: numpy array hoặc torch tensor (n x n)
        top_k: số lượng edges giữ lại cho mỗi node (nếu muốn sparse)
        normalize: có chuẩn hóa kernel về [0,1] không
    Returns:
        adj_mat_sparse: torch sparse tensor
    """
    if isinstance(kernel_matrix, np.ndarray):
        K = torch.from_numpy(kernel_matrix).float()
    else:
        K = kernel_matrix.float()
    if normalize:
        K_min = K.min()
        K_max = K.max()
    ...
    K = K.clone()
    K.fill_diagonal_(0)
    # Giữ top_k edges lớn nhất cho mỗi node
    n = K.shape[0]
    adj = torch.zeros_like(K)
    for i in range(n):
        vals, idxs = torch.topk(K[i], top_k)
        adj[i, idxs] = K[i, idxs]
    # Đối xứng
    adj = torch.maximum(adj, adj.T)
    # Thêm self-loop nếu cần
    # adj += torch.eye(n)
    # Chuẩn hóa
    adj = F.normalize(adj, p=1)
    # Chuyển sang sparse
